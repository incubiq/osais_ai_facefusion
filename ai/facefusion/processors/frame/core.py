...
def update_progress(progress : Any = None) -> None:
	...
	status="Processing frames ("+str(progress.n)+"/"+str(progress.total)+")"
	_pcBy20=progress.n/progress.total
	bNotify=False
	if _pcBy20>0.05 and gNotifyProgress==0.35:
		bNotify=True
		gNotifyProgress=0.4
	if _pcBy20>0.2 and gNotifyProgress==0.4:
		bNotify=True
		gNotifyProgress=0.5
	if _pcBy20>0.4 and gNotifyProgress==0.5:
		bNotify=True
		gNotifyProgress=0.6
	if _pcBy20>0.6 and gNotifyProgress==0.6:
		bNotify=True
		gNotifyProgress=0.7
	if _pcBy20>0.8 and gNotifyProgress==0.7:
		bNotify=True
		gNotifyProgress=0.8
